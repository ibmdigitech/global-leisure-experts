...
export function QuoteRequestForm({ productSlug, onSuccess }: QuoteRequestFormProps) {
  ...
  if (submitSuccess) {
    return (
      ...
      >
        ...
        <h3 className="text-xl font-bold text-deep-navy mb-2">Quote Request Received</h3>
        <p className="text-secondary-text">Thank you for your interest. Our team will contact you with pricing and availability details.</p>
      </motion.div>
    );
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="sm:col-span-2">
          <label htmlFor="productInterest" className="block text-sm font-medium text-primary-text mb-2">
            Product Interest <span className="text-safety-amber">*</span>
          </label>
          <select
          ...
          {errors.productInterest && (
            <p className="mt-1.5 text-sm text-red-600">{errors.productInterest.message}</p>
          )}
        </div>

        <div>
          <label htmlFor="quantity" className="block text-sm font-medium text-primary-text mb-2">
            Quantity
          </label>
          <input
            {...register('quantity')}
